...
//base url
String url =
    'https://awiiapp.com/'; //please add '/' at the end of url as 'yourwebsite.com/'
String mapkey = 'AIzaSyABpOyFBpcgHDlAGSKB8AG3owo84rUIcNU';
...
getDetailsOfDevice() async {
  ...
  try {
    rootBundle.loadString('assets/maps_style.json').then((value) {
      mapStyle = value;
    });
  ...
  try {
    var response = await http.post(Uri.parse('${url}api/v1/request/eta'),
        ...
        body: jsonEncode({
          'pick_lat': (userRequestData.isNotEmpty)
              ...
              : addressList.firstWhere((e) => e.id == '1').latlng.latitude,
          'pick_lng': (userRequestData.isNotEmpty)
              ? userRequestData['pick_lng']
              : addressList.firstWhere((e) => e.id == '1').latlng.longitude,
          'drop_lat': (userRequestData.isNotEmpty)
              ? userRequestData['drop_lat']
              : addressList
                  .firstWhere((e) => e.id == addressList.length.toString())
                  .latlng
                  .latitude,
    ...
    if (response.statusCode == 200) {
      ...
      if (choosenVehicle != null) {
        vehicleIconType =
            etaDetails[choosenVehicle == -1 ? 0 : choosenVehicle]['id'];
      }
      result = true;
      valueNotifierBook.incrementNotifier();
    } else {
...
createRequest() async {
  print(addressList[0].latlng.latitude);
  print(addressList[0].latlng.longitude);
  dynamic result;
  try {
...
getGoodsList() async {
  ...
  try {
    var response = await http.get(Uri.parse(
        '${url}api/v1/common/goods-types?vehicle_type=$vehicleIconType'));
    if (response.statusCode == 200) {
